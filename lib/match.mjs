 ...
 *
 * Para as seleções da CPU é a força do Ranking FIFA, e pronto. Para a seleção do jogador a
 * convocação pesa — mas a régua é o MELHOR elenco que ele poderia ter convocado, não uma
 * escala absoluta: o Ranking FIFA do Brasil já pressupõe o Brasil jogando com os melhores
 * brasileiros. Levar os 23 melhores rende um bônus pequeno (o treinador acertou a lista);
 * deixar craque em casa custa bem mais do que isso.
 *
 * `squadAvg` e `bestAvg` são overalls médios; a conversão para pontos de força usa o fator
 * do seed (1 ponto de overall ≈ 1,8 de força).
 */
export const SQUAD_BONUS = 2;

export function teamRating(strength, squadAvg, bestAvg) {
  if (!squadAvg || !bestAvg) return strength;
  const penalty = clamp((squadAvg - bestAvg) * 1.8, -10, 0);
...
export function expectedGoalsFor(rngHome, rngAway, neutral) {
  ...
  return [
    clamp(BASE_HOME * Math.exp(SPREAD * diff), 0.12, 6.5),
    clamp(BASE_AWAY * Math.exp(-SPREAD * diff), 0.08, 6.5),
  ];
}
